...
<a name="componentsAuthorizationDetail"></a> authorizationDetails | Map[`string`, [Authorization Detail Object](#authorizationDetail) \| [Reference Object](#referenceObject)] | An object to hold reusable [Authorization Detail Object](#authorizationDetail).
...
<a name=""></a> authorization_details | Map[[Authorization Detail Objects](#authorizationDetail), [Reference Object](#referenceObject)]  | `oauth2` | An array of [Authorization Detail Objects](#authorizationDetail)  which further refine the authorization requirements for this OAuth Flow, following the structure defined in the [Rich Authorization Requests (RFC 9126)](https://datatracker.ietf.org/doc/html/rfc9126). This is only applicable to the "authorizationCode" flow.|
...
Field Pattern | Type | Description
---|:---:|---
<a name="securityRequirementsName"></a>{name} | [`string` \| [Authorization Detail Object](#authorizationDetail) \| [Reference Object](#referenceObject) ] | Each name MUST correspond to a security scheme which is declared in the [Security Schemes](#componentsSecuritySchemes) under the [Components Object](#componentsObject). If the security scheme is of type `"oauth2"` or `"openIdConnect"`, then the value is a list of scope names OR  [Authorization Detail Objects](#authorizationDetail) (or a reference to one) required for the execution, and the list MAY be empty if authorization does not require a specified scope or authorization_details. For other security scheme types, the array MAY contain a list of role names which are required for the execution, but are not otherwise defined or exchanged in-band.
  ...
  "security": [
    {},
    {
      "petstore_auth": [
        "write:pets",
        "read:pets",
        "$ref": "#/components/schemas/payment_initiation"
      ]
    }
  ]
}
...
security:
  ...
  - petstore_auth:
    - write:pets
    - read:pets
    - $ref: '#/components/authorization/payment_initiation'
```
#### <a name="authorizationDetail"></a> Authorization Detail Object

Defines further details required for an operation's authorization as described in the [Rich Authorization Requests (RFC 9126)](https://datatracker.ietf.org/doc/html/rfc9126). The `type` field is required, while the rest of the fields are optional and vary based on the `type` value.

| Field Name | Type | Description |
| --- | --- | --- |
| `type` | `string` | REQUIRED. The type of the additional detail. This determines the required and optional fields for the detail object as per [RFC 9126](https://datatracker.ietf.org/doc/html/rfc9126). |
| `...` | `[any]` | Depending on the `type`, additional properties may be required or optional. These additional properties and their types are not defined by this specification, but should follow the guidance in [RFC 9126](https://datatracker.ietf.org/doc/html/rfc9126). |
